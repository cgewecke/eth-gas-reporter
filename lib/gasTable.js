  ...
  generate(info) {
    ...
    _.forEach(info.methods, (data, methodId) => {
      ...
      if (data.gasData.length) {
        ...
        stats.cost = (this.config.ethPrice && this.config.gasPrice)
          ? utils.gasToCost(stats.average, this.config.ethPrice, this.config.gasPrice)
          : colors.grey('-')
    ...
    info.deployments.forEach(contract => {
      ...
      stats.percent = utils.gasToPercentOfLimit(stats.average, this.config.blockLimit)
    ...
    if (this.config.ethPrice && this.config.gasPrice) {
      ...
      methodSubtitle = [
        ...
        {hAlign: 'center', colSpan: 2, content: colors.red(`${rate} ${currency}/eth`)}
      ]
    } else {
