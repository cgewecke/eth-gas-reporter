  ...
  generate(info) {
    ...
    _.forEach(info.methods, (data, methodId) => {
      ...
      if (data.gasData.length) {
        stats.total = data.gasData.reduce((acc, datum) => acc + datum, 0);

        stats.average = Math.round(stats.total / data.gasData.length);
        ...
        stats.total = colors.grey("-");
        stats.totalCost = colors.grey("-");
      }
      ...
      if (!this.config.onlyCalledMethods || data.numberOfCalls > 0) {
        ...
        section.push({ hAlign: "right", content: stats.total });
        section.push({ hAlign: "right", content: stats.totalCost });
    ...
    info.deployments.forEach(contract => {
      ...
      stats.total = contract.gasData.reduce((acc, datum) => acc + datum, 0);;
      stats.average = Math.round(stats.total / contract.gasData.length);
    ...
    const header = [
      ...
      colors.green("Total gas"),
      colors.green("Total cost"),
