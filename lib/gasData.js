...
class GasData {
  ...
  initialize(config) {
    ...
    // Shelljs will normalize the path
    ...
    files.forEach(file => {
      ...
      utils
        ...
        .forEach( name => {
          ...
          let contract;
          try { contract = artifactor.require(name) } catch (error) {return}
  ...
  getContractByDeploymentInput(input){
    const matches = this.deployments.filter(item => utils.matchBinaries(input, item.bytecode));
