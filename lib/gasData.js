...
const sync = require('./syncRequest');
...
class GasData {
  ...
  initialize(truffleArtifacts, config) {
    ...
    const files = shell.ls(`./${config.srcPath}/**/*.sol`)
    ...
    files.forEach(file => {
      utils.getContractNames(file).forEach(name => {
        ...
        const contractInfo = {
          name: name,
          binary: contract.unlinked_binary, // TRUFFLE SPECIFIC
          gasData: []
        }
        ...
        if (deployed && deployed.transactionHash) {
          ...
          const receipt = sync.getTransactionReceipt(deployed.transactionHash);
        ...
        Object.keys(methodIDs).forEach(key => {
          const isInterface = contract.unlinked_binary === '0x'; // TRUFFLE SPECIFIC
