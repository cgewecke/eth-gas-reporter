...
class Config{
  ...
  resolveClientUrl(options){
    ...
    if (web3 && web3.currentProvider){
      ...
      if(cp.connection)
        return cp.connection.url.replace('ws://', 'http://')
      ...
      if(
        cp._provider &&
        cp._provider.provider &&
        cp._provider.provider.host
      )
        return cp._provider.provider.host
    }

    // Case: Failure
    const msg = `ERROR: eth-gas-reporter was unable to resolve a client url ` +
