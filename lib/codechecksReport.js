...
const hook = require("./codechecksHook");
const utils = require("./utils");

class CodeChecksReport {
  constructor(config) {
    this.config = config;
    this.namespace = "cgewecke/eth-gas-reporter";
    this.increases = 0;
    this.decreases = 0;
   ...
   */
  generate(info) {
    if (!this.config.codechecks || !process.env.CI) return;
    ...
    let title = [
      ...
      },
      {
        hAlign: "center",
        colSpan: 2,
        content: `Block limit: ${info.blockLimit} gas`
      }
    ];
    ...
    // ---------------------------------------------------------------------------------------------
    let report = "\n```diff\n" + table.toString() + "\n```";
    this.saveCodeChecksData(report);
  }

  getMethodDiff(id, currentVal) {
    const previousVal = this.oldData.methods[id];
  ...
  getDeploymentDiff(id, currentVal) {
    const previousVal = this.oldData.deployments[id];
    const diff = typeof previousVal === "number" ? currentVal - previousVal : 0;
  ...
  async loadData() {
    if (
      this.config.codechecks &&
      process.env.CI &&
      typeof codechecks.getValue === "function"
    ) {
      this.oldData = await codechecks.getValue(this.namespace);
    }
  }

  saveCodeChecksData(report) {
    const output = {
      namespace: this.namespace,
      newData: this.newData,
      success: {
        name: "Gas Usage",
        shortDescriptions: `${this.getShortDescription()}`,
        longDescription: report
      }
    };

    fs.writeFileSync("./codechecksReport.json", JSON.stringify(output));
    fs.writeFileSync("./codechecks.js", hook);
  }
}
