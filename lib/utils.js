...
const utils = {
   ...
   * @return {Bool}
   */
  matchBinaries: function (input, binary) {
    const regExp = utils.bytecodeToBytecodeRegex(binary);
    return (input.match(regExp) !== null);
  },

  /**
   * Generate a regular expression string which is library link agnostic so we can match
   * linked binary deployment transaction inputs to the evm.bytecode solc output.
